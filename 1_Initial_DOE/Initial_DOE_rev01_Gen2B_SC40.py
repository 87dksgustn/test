...
def apply_bias_to_unit_column(unit_col, direction, strength, target_unit=None):
    ...
    if direction_norm == "target":
        if target_unit is None:
            raise ValueError("target bias에는 target_unit 값이 필요합니다.")

        target_unit = float(np.clip(target_unit, 0.0, 1.0))

        if np.isclose(target_unit, 0.0):
            return np.power(unit_col, strength_value)

        if np.isclose(target_unit, 1.0):
            return 1.0 - np.power(1.0 - unit_col, strength_value)
    ...
    raise ValueError(
        ...
        "사용 가능: high, low, center, edge, target"
    )
        ...
        if use_bias:
            ...
            if rule is not None:
                ...
                if direction == "target":
                    ...
                    target_unit = (target_value - lower) / (upper - lower)

                unit_col = apply_bias_to_unit_column(
                    unit_col,
                    direction,
                    strength,
...
def compute_bias_adherence_score(df_unit, bias_rules):
    ...
    for var_name, rule in bias_rules.items():
        ...
        if direction == "high":
            ...
            target_value = float(rule["target_value"])
            target_unit = float(np.clip((target_value - lower) / (upper - lower), 0.0, 1.0))
            score = float(np.clip(1.0 - (np.mean(np.abs(values - target_unit)) / 0.5), 0.0, 1.0))
        else:
            score = 0.5
