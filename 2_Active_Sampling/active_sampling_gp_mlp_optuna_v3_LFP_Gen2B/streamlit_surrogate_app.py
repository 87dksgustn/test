...
def single_predict_ui(bundle):
    ...
    with st.form("single_predict_form"):
        ...
        A_Cell_D = c1.number_input("A_Cell_D", min_value=float(a_lo), max_value=float(a_hi), value=float((a_lo + a_hi) / 2), step=0.01)
        C_Barrier_Thx = c1.number_input("C_Barrier_Thx", min_value=float(c_lo), max_value=float(c_hi), value=float((c_lo + c_hi) / 2), step=0.01)
        E_Barrier_Outer_Thx = c2.number_input("E_Barrier_Outer_Thx", min_value=float(e_lo), max_value=float(e_hi), value=float((e_lo + e_hi) / 2), step=0.01)
        F_ThermalResin_Thx = c2.number_input("F_ThermalResin_Thx", min_value=float(f_lo), max_value=float(f_hi), value=float((f_lo + f_hi) / 2), step=0.01)

        b_levels = levels.get("B_Barrier_Type", ["Si1"])
        d_levels = levels.get("D_Barrier_Outer_Type", ["PU", "Si1"])
        B_Barrier_Type = c3.selectbox("B_Barrier_Type", options=b_levels, index=0)
        D_Barrier_Outer_Type = c3.selectbox("D_Barrier_Outer_Type", options=d_levels, index=0)
