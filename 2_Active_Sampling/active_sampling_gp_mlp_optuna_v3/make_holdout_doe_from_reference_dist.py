...
REFERENCE_CSV = Path(r"C:\Users\hyunsoo.an\Desktop\Barrier_Stop_Tool\2_Active_Sampling\active_sampling_gp_mlp_optuna_v3_LFP_Gen2B\outputs\final_test_doe\initial_DOE_best_seed_greedy_maximin.csv")
N_OUTPUT = 50
RANDOM_SEED = 42
OVERSAMPLE_FACTOR = 120  # Build a larger candidate pool then pick maximin subset.
...
DISCRETE_ALIAS = {
    "B_Barrier_Type": ["B_Barrier_Type", "Barrier_Type"],
    "D_Barrier_Outer_Type": ["D_Barrier_Outer_Type", "Barrier_Outer_Type"],
}
...
def greedy_maximin_subset(df_cont: pd.DataFrame, n_pick: int, seed: int) -> np.ndarray:
    ...
    # Start from point nearest to medoid-like center for stability.
...
def main() -> None:
    ...
    # Distribution-following pool: bootstrap + small jitter + clipping to continuous bounds.
    ...
    # Discrete policy A: assign based on current config level ratios (uniform over declared levels).
    ...
    report = {
        ...
        "source_column_mapping": {
            "continuous": cont_src,
            "discrete": {
                k: _pick_existing_col(ref.columns.tolist(), v)
                for k, v in DISCRETE_ALIAS.items()
                if any(c in ref.columns for c in v)
            },
        },
    ...
    print(out[base_cols + config.DISCRETE_COLS].head(10).to_string(index=False))
